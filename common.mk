...
# add shutdown command from pta - ursus schneider - 25.06.2017
CFG_WITH_SHUTDOWN := y
...
optee-os-clean-common: xtest-clean helloworld-clean fim-clean rsa-clean
...
################################################################################
# FIM -> File Integraty Monitoring
################################################################################
FIM_COMMON_FLAGS ?= HOST_CROSS_COMPILE=$(CROSS_COMPILE_NS_USER)\
	TA_CROSS_COMPILE=$(CROSS_COMPILE_S_USER) \
	TA_DEV_KIT_DIR=$(OPTEE_OS_TA_DEV_KIT_DIR) \
	TEEC_EXPORT=$(OPTEE_CLIENT_EXPORT)

fim-common: optee-os optee-client
	$(MAKE) -C $(FIM_PATH) $(FIM_COMMON_FLAGS)

FIM_CLEAN_COMMON_FLAGS ?= TA_DEV_KIT_DIR=$(OPTEE_OS_TA_DEV_KIT_DIR)

fim-clean-common:
	$(MAKE) -C $(FIM_PATH) $(FIM_CLEAN_COMMON_FLAGS) clean

################################################################################
# RSA test
################################################################################
RSA_COMMON_FLAGS ?= HOST_CROSS_COMPILE=$(CROSS_COMPILE_NS_USER)\
	TA_CROSS_COMPILE=$(CROSS_COMPILE_S_USER) \
	TA_DEV_KIT_DIR=$(OPTEE_OS_TA_DEV_KIT_DIR) \
	TEEC_EXPORT=$(OPTEE_CLIENT_EXPORT)

rsa-common: optee-os optee-client
	$(MAKE) -C $(RSA_PATH) $(RSA_COMMON_FLAGS)

RSA_CLEAN_COMMON_FLAGS ?= TA_DEV_KIT_DIR=$(OPTEE_OS_TA_DEV_KIT_DIR)

rsa-clean-common:
	$(MAKE) -C $(RSA_PATH) $(RSA_CLEAN_COMMON_FLAGS) clean
...
filelist-tee-common: optee-client xtest helloworld fim rsa
	...
	@if [ -e $(FIM_PATH)/host/fim ]; then \
		echo "file /bin/fim" \
			"$(FIM_PATH)/host/fim 755 0 0"	>> $(fl); \
		echo "file /lib/optee_armtz/361142ae-ec3e-4576-8883-a6c0f6c6ec71.ta" \
			"$(FIM_PATH)/ta/361142ae-ec3e-4576-8883-a6c0f6c6ec71.ta" \
			"444 0 0" 					>> $(fl); \
			echo "file /bin/testfile1.txt" \
				"$(FIM_PATH)/testfiles/testfile1.txt 755 0 0"	>> $(fl); \
				echo "file /bin/testfile2.txt" \
					"$(FIM_PATH)/testfiles/testfile2.txt 755 0 0"	>> $(fl); \
					echo "file /bin/testfile3.txt" \
						"$(FIM_PATH)/testfiles/testfile3.txt 755 0 0"	>> $(fl); \
	fi
	@if [ -e $(RSA_PATH)/host/rsa ]; then \
		echo "file /bin/rsa" \
			"$(RSA_PATH)/host/rsa 755 0 0"	>> $(fl); \
		echo "file /lib/optee_armtz/d6f85138-131c-4f86-b746-a066b2536e69.ta" \
			"$(RSA_PATH)/ta/d6f85138-131c-4f86-b746-a066b2536e69.ta" \
			"444 0 0" 					>> $(fl); \
	fi
